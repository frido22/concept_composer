...
const SyncNode = memo(({ data, selected }: any) => {
  const { sync } = data as SyncNodeData;
    ...
    <div className={`bg-[#384959] border-2 border-[#384959] rounded-none p-4 min-w-64 shadow-lg ${selected ? 'shadow-xl border-[#88BDF2]' : ''}`}>
      ...
      {sync.when.length > 0 && (
        <div className="mb-3">
          ...
          {sync.when.map((pattern: any, index: number) => (
      ...
      {sync.where.length > 0 && (
        <div className="mb-3">
          ...
          {sync.where.map((step: any, index: number) => (
            <div key={index} className="text-xs text-gray-300 mb-1">
      ...
      {sync.then.length > 0 && (
        <div className="mb-3">
          ...
          {sync.then.map((pattern: any, index: number) => (
            <div key={index} className="text-xs text-gray-300 mb-1">
              {pattern.conceptId.split(':')[1]}.{pattern.action}
            </div>
          ))}
